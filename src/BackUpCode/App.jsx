...
function Game() {
  ...
  function characterHover(event, over = false, out = false, index, player) {
    if (!gameOver) return;

    // When mouse In add Hover player
    if (over) {
      if (player === player1) {
        setPlayer1Hover(allCharacters[index]);
      } else if (player === player2) {
        setPlayer2Hover(allCharacters[index]);
      }
    }
  ...
  function pickPlayer1(index) {
    ...
    setPlayerStatusClass('player1Selected');
    // console.log(player1);
  ...
  function pickPlayer2(index) {
    ...
    if (player1 != nonePlayer1) {
      setPlayerStatusClass('player1Selected player2Selected'); // For mobile
    ...
    return (
      <li key={'C' + index} className={squaresP1[index].status}>
        <button
          ...
          onMouseOver={(event) =>
            characterHover(event, true, false, index, player1)
          }
          onMouseOut={(event) =>
            characterHover(event, false, true, index, player1)
          }
    ...
    return (
      <li key={'C' + index} className={squaresP2[index].status}>
        <button
          ...
          onMouseOver={(event) =>
            characterHover(event, true, false, index, player2)
          }
          onMouseOut={(event) =>
            characterHover(event, false, true, index, player2)
          }
          disabled={disableCharacter}
        >
          <img src={char.url} alt={char.alt} />
  ...
  function PlayerPickerP1({ className, text }) {
    return (
      <>
  ...
  function PlayerPickerP2({ className, text }) {
    return (
      <>
        <div className={className}>
          <h2>{text}</h2>
  ...
  function onPlay(index) {
    ...
    if (winner) {
      ...
      history.push(gamePoint); // Simpen History si pemenang | bisa kek gini tapi lebih bagus make concat() | lupakan kalo make concat ada delay
      // console.log(history);
  ...
  // Components
  function BackButton({ classStatus, onClick, id, text }) {
    return (
      <div className={`buttonStart backButton ${classStatus}`} id={id}>
  ...
  // Back handleClick
  function handleBack() {
    setPlayerStatusClass('player1NonSelected');
  }
  ...
  function backToLobby() {
    ...
    if (!gameOver) {
      ...
      // Reset for Mobile
      setPlayerStatusClass('player1NonSelected'); // For mobile
      ...
      <div className="bodyContainer">
        <div className="mobilePlayerPickerContainer">
          <PlayerPickerP1
            ...
            text={`PLAYER 1 (${
              player1Hover != null ? player1Hover.name : player1.name
            })`}
            disabled={true}
          />
          ...
          <PlayerPickerP2
            ...
            text={`PLAYER 2 (${
              player2Hover != null ? player2Hover.name : player2.name
            })`}
            disabled={true}
          />

          <BackButton
            classStatus={classStatus + ` ${playerStatusClass}`}
            id="backToP1Button"
            text="Back"
            onClick={handleBack}
          />
